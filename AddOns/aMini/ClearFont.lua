...
-- 按鈕（不可選狀態）
	...
	if (CanSetFont(GameFontDisable)) then 				GameFontDisable:SetTextColor(0.6, 0.6, 0.6); end	-- 預設值：(0.5, 0.5, 0.5)
...
-- PvP面板描述，團隊面板按鈕等
	...
	if (CanSetFont(GameFontDisableSmall)) then			GameFontDisableSmall:SetTextColor(0.6, 0.6, 0.6); end	-- 預設值：(0.5, 0.5, 0.5)

-- （未確認）
	if (CanSetFont(GameFontDarkGraySmall)) then 		GameFontDarkGraySmall:SetFont(CLEAR_FONT, 12 * CF_SCALE); end	-- 預設值：15
	if (CanSetFont(GameFontDarkGraySmall)) then 		GameFontDarkGraySmall:SetTextColor(0.4, 0.4, 0.4); end	-- 預設值：(0.35, 0.35, 0.35)
...
-- 競技場面板
	...
	if (CanSetFont(GameFontDisableLarge)) then			GameFontDisableLarge:SetTextColor(0.6, 0.6, 0.6); end	-- 預設值：(0.5, 0.5, 0.5)
...
-- 聊天輸入框字體
	if (CanSetFont(ChatFontNormal)) then 				ChatFontNormal:SetFont(CLEAR_FONT_CHAT, 14 * CF_SCALE, "OUTLINE"); end	-- 預設值：14
...
-- 聊天視窗默認字體
	if (CanSetFont(ChatFontSmall)) then 				ChatFontSmall:SetFont(CLEAR_FONT_CHAT, 13 * CF_SCALE, "OUTLINE"); end	-- 預設值：12
...
-- -----------------------------------------------------------------------------
	...
	if (CanSetFont(CombatTextFont)) then				CombatTextFont:SetFont(CLEAR_FONT_DAMAGE, 16 * CF_SCALE); end		-- 預設值：25
...
-- 任務標題
	...
	--if (CanSetFont(QuestTitleFont)) then 				QuestTitleFont:SetShadowColor(0.54, 0.4, 0.1); end		-- 預設值：(0, 0, 0)

-- 任務描述
	if (CanSetFont(QuestFont)) then 		   			QuestFont:SetFont(CLEAR_FONT_QUEST, 14 * CF_SCALE); end		-- 預設值：14
	--if (CanSetFont(QuestFont)) then 		   			QuestFont:SetTextColor(0.15, 0.09, 0.04); end			-- 預設值：(0, 0, 0)

-- 任務目標
	if (CanSetFont(QuestFontNormalSmall)) then			QuestFontNormalSmall:SetFont(CLEAR_FONT, 12 * CF_SCALE); end	-- 預設值：14
	--if (CanSetFont(QuestFontNormalSmall)) then			QuestFontNormalSmall:SetShadowColor(0.54, 0.4, 0.1); end	-- 預設值：(0.3, 0.18, 0)
...
-- -----------------------------------------------------------------------------
	...
	if (CanSetFont(ItemTextFontNormal)) then 	 	  	ItemTextFontNormal:SetFont(CLEAR_FONT_ITEM, 15 * CF_SCALE); end		-- 預設值：15
...
-- -----------------------------------------------------------------------------
	...
	--if (CanSetFont(MailTextFontNormal)) then 		   	MailTextFontNormal:SetTextColor(0.15, 0.09, 0.04); end		-- 預設值：(0.18, 0.12, 0.06)
...
-- -----------------------------------------------------------------------------
	...
	--if (CanSetFont(InvoiceTextFontNormal)) then 	   	InvoiceTextFontNormal:SetTextColor(0.15, 0.09, 0.04); end	-- 預設值：(0.18, 0.12, 0.06)

	if (CanSetFont(InvoiceTextFontSmall)) then			InvoiceTextFontSmall:SetFont(CLEAR_FONT_QUEST, 11 * CF_SCALE); end	-- 預設值：10
	--if (CanSetFont(InvoiceTextFontSmall)) then			InvoiceTextFontSmall:SetTextColor(0.15, 0.09, 0.04); end	-- 預設值：(0.18, 0.12, 0.06)
...
-- -----------------------------------------------------------------------------
	...
	if (CanSetFont(WorldMapTextFont)) then				WorldMapTextFont:SetShadowColor(0, 0, 0); end	-- 預設值：(1.0, 0.9294, 0.7607)
...
-- 大區域名
	...
	if (CanSetFont(ZoneTextFont)) then 	   			ZoneTextFont:SetShadowColor(0, 0, 0); end	-- 預設值：(1.0, 0.9294, 0.7607)
