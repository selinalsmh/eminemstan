...
local NATIVE_PLATE_WIDTH, NATIVE_PLATE_HEIGHT, NATIVE_CASTBAR_HEIGHT = 90, 9, 11
local NATIVE_FONT = STANDARD_TEXT_FONT --select(1, ChatFrame1:GetFont())
...
local CreateShadow = function(self)
	self.shadow = self.shadow or CreateFrame('Frame', nil, self)
	self.shadow:SetScale(UIParent:GetScale())
	self.shadow:SetPoint('TOPLEFT', self, 'TOPLEFT', -2, 1)
	self.shadow:SetPoint('BOTTOMRIGHT', self, 'BOTTOMRIGHT', 4, -4)
	self.shadow:SetFrameStrata('LOW')
	self.shadow:SetBackdrop({ bgFile = 'Interface\\Tooltips\\UI-Tooltip-Background', edgeFile = 'Interface\\Tooltips\\UI-Tooltip-Border',  tiled = false, edgeSize = 16, insets = {left=3, right=3, top=3, bottom=3} })
	self.shadow:SetBackdropColor(0, 0, 0, .3)
...
local CreateAuraIcon = function(parent)
	...
	button.border:SetPoint('TOPLEFT',button,'TOPLEFT', mult,-mult)
	button.border:SetPoint('BOTTOMRIGHT',button,'BOTTOMRIGHT',-mult,mult)
	
	button.icon = button:CreateTexture(nil, 'BORDER')
	button.icon:SetPoint('TOPLEFT',button,'TOPLEFT', mult*2,-mult*2)
	button.icon:SetPoint('BOTTOMRIGHT',button,'BOTTOMRIGHT',-mult*2,mult*2)
	...
	button.expired:SetFont(NATIVE_FONT, 9, 'OUTLINE')
	button.expired:SetShadowColor(0, 0, 0, 0)
	
	button.stack = button:CreateFontString(nil,'OVERLAY')
	button.stack:SetFont(NATIVE_FONT, 7, 'OUTLINE')
...
local CreateVirtualFrame = function(parent, point)
	...
	point.backdrop:SetPoint('TOPLEFT', point, 'TOPLEFT', -mult*3, mult*3)
	point.backdrop:SetPoint('BOTTOMRIGHT', point, 'BOTTOMRIGHT', mult*3, -mult*3)
	point.backdrop:SetTexture(unpack(background_color))
	point.backdrop:SetDrawLayer('BORDER', -8)

	if point.GetValue then
		point.light = parent:CreateTexture(nil, 'BORDER')
		point.light:SetPoint('TOPLEFT', point, 'TOPLEFT', -mult*5, mult*5)
		point.light:SetPoint('BOTTOMRIGHT', point, 'BOTTOMRIGHT', mult*5, -mult*5)
		point.light:SetTexture('Interface\\AddOns\\'..addonName..'\\highlight')
		point.light:SetDrawLayer('ARTWORK', 7)
	end
	
	point.bordertop = parent:CreateTexture(nil, "BORDER")
	point.bordertop:SetPoint("TOPLEFT", point, "TOPLEFT", -mult*2, mult*2)
	point.bordertop:SetPoint("TOPRIGHT", point, "TOPRIGHT", mult*2, mult*2)
	point.bordertop:SetHeight(mult)
	point.bordertop:SetTexture(unpack(background_color))
	point.bordertop:SetDrawLayer("BORDER", -7)
	
	point.borderbottom = parent:CreateTexture(nil, "BORDER")
	point.borderbottom:SetPoint("BOTTOMLEFT", point, "BOTTOMLEFT", -mult*2, -mult*2)
	point.borderbottom:SetPoint("BOTTOMRIGHT", point, "BOTTOMRIGHT", mult*2, -mult*2)
	point.borderbottom:SetHeight(mult)
	point.borderbottom:SetTexture(unpack(background_color))
	point.borderbottom:SetDrawLayer("BORDER", -7)
	
	point.borderleft = parent:CreateTexture(nil, "BORDER")
	point.borderleft:SetPoint("TOPLEFT", point, "TOPLEFT", -mult*2, mult*2)
	point.borderleft:SetPoint("BOTTOMLEFT", point, "BOTTOMLEFT", mult*2, -mult*2)
	point.borderleft:SetWidth(mult)
	point.borderleft:SetTexture(unpack(background_color))	
	point.borderleft:SetDrawLayer("BORDER", -7)
	
	point.borderright = parent:CreateTexture(nil, "BORDER")
	point.borderright:SetPoint("TOPRIGHT", point, "TOPRIGHT", mult*2, mult*2)
	point.borderright:SetPoint("BOTTOMRIGHT", point, "BOTTOMRIGHT", -mult*2, -mult*2)
	point.borderright:SetWidth(mult)
	point.borderright:SetTexture(unpack(background_color))
...
local UpdateThreat = function(self)
	...
	if self.threat:IsShown() then
		...
		if val > .7 then
			SetVirtualBorderColor(self.health, 1, .5, 0)
			...
			SetVirtualBorderColor(self.health, 1, 0, 0)
			self.health:SetSize(NATIVE_PLATE_WIDTH, NATIVE_PLATE_HEIGHT * 1.2)
		end
	else
...
local CastBar_OnShow = function(self)
	self:ClearAllPoints()
	self:SetSize(self:GetParent().health:GetWidth(), NATIVE_CASTBAR_HEIGHT)
	self:SetPoint('TOP', self:GetParent().health, 'BOTTOM', 0, -8)
	self:SetStatusBarTexture('Interface\\AddOns\\'..addonName..'\\texture')
...
local HealthBar_OnShow = function(self)
	...
	self.health.backdrop:SetPoint('TOPLEFT', -mult*3, mult*3)
	self.health.backdrop:SetPoint('BOTTOMRIGHT', mult*3, -mult*3)
...
local StylePlate = function(self)
	...
	if not self.health then
		self.health = health
		self.health:SetStatusBarTexture('Interface\\AddOns\\'..addonName..'\\texture')
	...
	if not self.health.value then
		...
		self.health.value:SetFont(NATIVE_FONT, 9, 'OUTLINE')
	end
	
	if not self.health.level then
		self.health.level = level
		self.health.level:SetFont(NATIVE_FONT, 10, 'OUTLINE')
	...
	if not self.health.name then
		...
		self.health.name:SetFont(NATIVE_FONT, 10, 'OUTLINE')
	...
	if not self.castbar.time then
		...
		self.castbar.time:SetFont(NATIVE_FONT, 10, 'OUTLINE')
	...
	if not self.castbar.name then
		...
		self.castbar.name:SetFont(NATIVE_FONT, 10, 'OUTLINE')
	...
	if not self.AuraWidget then
		...
		for index = 1, MAX_DISPLAYABLE_DEBUFFS do AuraIconFrames[index] = CreateAuraIcon(self.AuraWidget)  end
		...
		self.AuraWidget:SetScript('OnHide', function() for index = 1, MAX_DISPLAYABLE_DEBUFFS do ScheduleHide(AuraIconFrames[index], 0) end end)	
	end
...
bNamePlates:RegisterEvent('PLAYER_LOGIN')
