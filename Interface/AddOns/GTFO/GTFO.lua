...
Special thanks: 
		...
		Wowuicn and xazhaoyang (Simplified and Traditional Chinese Translations)
		Blubibulga and TrAsHeR (French Translations)
		Sunyruru (Korean Translations)
		Pablous (Spanish Translations)
		lsjyzjl, Andyca, and BNSSNB (Traditional Chinese Translations)
...
GTFO = {
	...
	Version = "4.17"; -- Version number (text format)
	VersionNumber = 41700; -- Numeric version number for checking out-of-date clients
	...
	ShowAlert = nil;
	Settings = { };
	TrivialDamagePercent = .5; -- Minimum % of HP required for an alert to be trivial
	UIRendered = nil;
	...
	if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
				...
				if (sourceGUID == UnitGUID("player")) then
					...
					if (SpellType=="SPELL_DAMAGE") then
						...
						if (GTFO.FFSpellID[SpellID]) then
							...
							if (GTFO.FFSpellID[SpellID].trivialPercent and not GTFO.Settings.TrivialMode) then
								local damagePercent = (damage * 100) / UnitHealthMax("player");
							 	if ((GTFO.FFSpellID[SpellID].trivialPercent == 0 and damagePercent < GTFO.TrivialDamagePercent) or (GTFO.FFSpellID[SpellID].trivialPercent > 0 and damagePercent < GTFO.FFSpellID[SpellID].trivialPercent)) then
		...
		if (SpellType == "ENVIRONMENTAL_DAMAGE") then
			...
			elseif (environment == "LAVA") then
				...
				if (not GTFO.Settings.TrivialMode and damagePercent < GTFO.TrivialDamagePercent) then
					-- Trivial
		...
		elseif (SpellType=="SPELL_PERIODIC_DAMAGE" or SpellType=="SPELL_DAMAGE" or SpellType=="SPELL_MISSED" or SpellType=="SPELL_PERIODIC_MISSED" or SpellType=="SPELL_ENERGIZE" or SpellType=="SPELL_INSTAKILL" or ((SpellType=="SPELL_AURA_APPLIED" or SpellType=="SPELL_AURA_APPLIED_DOSE" or SpellType=="SPELL_AURA_REFRESH") and misc4=="DEBUFF")) then
			...
			if (SpellID ~= "124255" and SpellID ~= "124275") then -- Hide Monk's "Stagger"
				if (vehicle) then
			...
			if (GTFO.SpellID[SpellID]) then
				...
				if (not GTFO.Settings.TrivialMode) then
					...
					if (GTFO.SpellID[SpellID].trivialPercent and (SpellType=="SPELL_PERIODIC_DAMAGE" or SpellType=="SPELL_DAMAGE")) then
						local damagePercent = (damage * 100) / UnitHealthMax("player");
					 	if ((GTFO.SpellID[SpellID].trivialPercent == 0 and damagePercent < GTFO.TrivialDamagePercent) or (GTFO.SpellID[SpellID].trivialPercent > 0 and damagePercent < GTFO.SpellID[SpellID].trivialPercent)) then
...
function GTFO_SendUpdateRequest()
	...
	if (UnitInBattleground("player")) then
		SendAddonMessage("GTFO","U:BATTLEGROUND","BATTLEGROUND");
