...
do
	...
	local loadPosition = function(self)
		...
		return self:SetPoint(p1, frame, p2, B.Scale(x), B.Scale(y))
	end
	...
	function Spawn(type)
		...
		bg:SetTexture(C.blank)
		bg:SetVertexColor(r * .2, g * .2, b * .2)
		
		local border = CreateFrame("Frame", nil, frame)
		border:SetPoint("TOPLEFT", frame)--, B.Scale(-2), B.Scale(2))
		border:SetPoint("BOTTOMRIGHT", frame)--, B.Scale(2), B.Scale(-2))
		border:CreateShadow("Background")
		border:SetFrameLevel(0)
	
		local text = frame:CreateFontString(nil, 'OVERLAY')
		text:SetFont(C.font, C.fontsize, C.fontflag)
