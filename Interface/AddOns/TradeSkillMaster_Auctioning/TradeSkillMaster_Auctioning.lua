...
local savedDBDefaults = {
	profile = {
		...
		ignoreRandomEnchant = {default = false},
		groups = {},
		categories = {},
	},
	global = {
...
function TSM:DoDBCleanUp()
	...
	-- move to new per-group option
	if TSM.db.profile.itemIDGroups then
		for groupName, value in pairs(TSM.db.profile.itemIDGroups) do
			if value and TSM.db.profile.groups[groupName] then
				TSM.db.profile.ignoreRandomEnchant[groupName] = true
				local newGroupValues = {}
				for itemID in pairs(TSM.db.profile.groups[groupName]) do
					newGroupValues[TSMAPI:GetItemString(itemID)] = true
				end
				TSM.db.profile.groups[groupName] = newGroupValues
			end
		end
	end
...
function TSM:UpdateItemReverseLookup()
	...
	for group, items in pairs(TSM.db.profile.groups) do
		local ignoreRandomEnchants = TSM.Config:GetBoolConfigValue(group, "ignoreRandomEnchant", true)
		for itemString in pairs(items) do
			if ignoreRandomEnchants then
				TSM.itemReverseLookup[TSMAPI:GetSafeItemID(itemString) or itemString] = group
			else
...
function TSM:TSMAUC_NEW_GROUP_ITEM(_, groupName, itemID, isNewGroup, category)
	itemID = itemID and select(2, TSMAPI:GetSafeItemInfo(itemID)) or itemID
	...
	if itemID then
		...
		if itemString then
			...
			local existingGroupName = TSM.itemReverseLookup[itemString]
...
function TSM:LoadTooltip(itemID)
	...
	itemID = TSMAPI:GetSafeItemID(itemString)
