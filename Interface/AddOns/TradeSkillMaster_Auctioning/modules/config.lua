...
function Config:DrawGroupGeneral(container, groupName)
	...
	local function OnIgnoreEnchantChange()
		if Config:GetBoolConfigValue(groupName, "ignoreRandomEnchant", true) then
			...
			for itemString in pairs(TSM.db.profile.groups[groupName]) do
				local itemID = TSMAPI:GetSafeItemID(itemString)
				if itemID then
					groupItems[itemID] = true
				end
			end
			
			for itemString, group in pairs(TSM.itemReverseLookup) do
				local itemID = TSMAPI:GetSafeItemID(itemString)
				if group ~= groupName and itemID and groupItems[itemID] then
					TSM.db.profile.groups[group][itemString] = nil
					local link = select(2,  TSMAPI:GetSafeItemInfo(itemString)) or itemString
					TSM:Printf(L["%s removed from '%s' as a result of setting the current group to ignore random enchants."], link, group)
				end
			end
		end
	end
			...
			children = {
				...
				{ -- General Settings
					...
					children = {
						...
						{
							type = "CheckBox",
							label = L["Ignore Random Enchants"],
							settingInfo = {"ignoreRandomEnchant", 1},
							relativeWidth = 0.49,
							tooltip = L["If checked, any items in this group with random enchantments will have their random enchantments ignore by Auctioning.\n\nNote: Any common search term will be ignored for groups with this box checked."],
							hidden = isDefaultPage,
							callback = OnIgnoreEnchantChange,
						},
					},
				},
				{ -- Spacer
...
function Config:DrawQuickCreation(container)
	...
	for groupName, items in pairs(TSM.db.profile.groups) do
		local ignoreRandomEnchants = Config:GetBoolConfigValue(groupName, "ignoreRandomEnchant", true)
		for itemString in pairs(items) do
			if ignoreRandomEnchants then
				itemString = TSMAPI:GetSafeItemID(itemString)
			end
			itemsInGroups[itemString] = true
		end
	end
	...
	for bag, slot, itemString in TSM:GetBagIterator(true) do
		if itemString and not TSM.Util:IsSoulbound(bag, slot) then
			local itemID = TSMAPI:GetSafeItemID(itemString)
			if not usedLinks[itemString] and not itemsInGroups[itemString] and not itemsInGroups[itemID] then
				local link = GetContainerItemLink(bag, slot)
				local name, _, quality, _, _, _, _, _, _, texture = TSMAPI:GetSafeItemInfo(link)
	...
	for itemString in pairs(selectedItems) do
		local name, link, quality, _, _, _, _, _, _, texture = TSMAPI:GetSafeItemInfo(itemString)
		if link then
			tinsert(itemsToAdd, {value=itemString, text=link, icon=texture, name=name, tooltip=link})
		end
	end
	...
	local function OnButtonClick()
		for itemString in pairs(selectedItems) do
			local itemName = strlower(TSMAPI:GetSafeItemInfo(itemString))
...
function Config:DrawAddRemoveItem(container, group)
	...
	local tempItems = {}
	for itemString in pairs(TSM.db.profile.groups[group]) do
		if Config:GetBoolConfigValue(group, "ignoreRandomEnchant", true) then
			itemString = TSMAPI:GetSafeItemID(itemString)
		end
		tempItems[itemString] = true
	end
	for itemString in pairs(tempItems) do
		local name, link, _, _, _, _, _, _, _, texture = TSMAPI:GetSafeItemInfo(itemString)
		if name then
	...
	local ignoreRandomEnchant = Config:GetBoolConfigValue(group, "ignoreRandomEnchant", true)
	for bag, slot, itemString in TSM:GetBagIterator(true) do
		if itemString and not TSM.Util:IsSoulbound(bag, slot) then
			local link = GetContainerItemLink(bag, slot)
			local itemID = TSMAPI:GetSafeItemID(link)
			if not usedLinks[itemString] and not TSM.itemReverseLookup[itemString] and not usedLinks[itemID] then
				if ignoreRandomEnchant then
					link = select(2, TSMAPI:GetSafeItemInfo(itemID)) or link
				end
			...
			children = {
				{
					...
					onRemove = function(_,_,selected)
							for i=#selected, 1, -1 do
								if Config:GetBoolConfigValue(group, "ignoreRandomEnchant", true) then
									local selectedItem = TSMAPI:GetSafeItemID(selected[i])
									for itemString in pairs(TSM.db.profile.groups[group]) do
										if selectedItem == TSMAPI:GetSafeItemID(itemString) then
											TSM.db.profile.groups[group][itemString] = nil
										end
									end
								else
