...
--[[
		if BagnonFrame then
			...
			hooksecurefunc(Bagnon, 'Create', function()
				...
				--BuildButton(f, 45, 20, 'TOPRIGHT', -30, -7):SetFrameStrata('TOOLTIP')
			end)
		else
		--]]
			local id = 1
			hooksecurefunc(Bagnon.Frame, 'New', function()
				local f = getglobal('BagnonFrameinventory')
				if not f then return end
				BuildButton(f, 45, 20, 'TOPRIGHT', -55, -8):SetFrameStrata('TOOLTIP')

				local f2 = getglobal('BagnonFramebank')
				if not f2 then return end
				BuildButton(f2, 45, 20, 'TOPRIGHT', -55, -8):SetFrameStrata('TOOLTIP')
