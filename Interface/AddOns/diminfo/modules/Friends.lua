...
if cfg.Friends == true then
	...
	local function Update(self, event, ...)
		...
		Text:SetText(format(cfg.ColorClass and "%s: "..init.Colored.."%d" or "%s:%d","好友",onlineFriends + numBNetOnline))
			...
			for i = 1, #BNTable do
				...
				if (info[8]) then
					...
					if info[7] == wowString and info[13] == select(1, UnitFactionGroup("player")) then
						...
						if classc == nil then classc = GetQuestDifficultyColor(info[17]) end
